...
class RobotTracker:
    ...
    def add_coordinate(self, step_count, left_sensor, right_sensor, left_speed, right_speed, detections=None):
        ...
        if detections:
            ...
            # Handle both old format (det['plant']) and new hybrid format (det['species'])
            plants_found = []
            for det in detections:
                if 'plant' in det:
                    plants_found.append(det['plant'])
                elif 'species' in det:
                    plants_found.append(det['species'])
                else:
                    plants_found.append('unknown')
            coordinate['plants_found'] = plants_found
